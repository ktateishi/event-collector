...
## Task 13: リマインドチェックの自動実行 — **完了（2026-08-13）**

**Description:** Task 12のリマインドチェックロジックを、Vercel Cronで毎日自動実行する。

**実装メモ**: `/api/reminders`は収集（07:00 JST）完了後の07:10 JST（`"10 22 * * *"`）に
実行するようスケジュール。収集Cron→自動LINE送信（Task 9）と違い、リマインド判定は
「今日収集したイベント」ではなく`registration_opens_at`/`deadline_at`という蓄積済みの
日付を見るだけなので、収集完了とのレースコンディションは発生しない（Task 9のような
同一リクエスト内呼び出しは不要、独立したCronで問題ない）。10分のバッファは
念のため収集完了後を確実にするため。

**Acceptance criteria:**
- [x] 毎日決まった時刻にリマインドチェックが自動実行される — `vercel.json`に
      `/api/reminders`のcronエントリ（`10 22 * * *` = 07:10 JST）を追加
- [x] 対象イベントがない日は何も送信されない — `selectEventsForReminder`が0件を返せば
      `sendBroadcast`は何もしない（Task 12のユニットテストで既に保証）

**Verification:**
- [ ] Vercelのcron実行ログでリマインドチェックが正常終了していることを確認する
      （次回07:10 JSTの自動実行後に確認予定）
...
## Task 16: ノイズ判定基準のチューニング — **プロンプト改善完了、定性確認は運用継続待ち（2026-08-13）**

**Description:** MVP運用開始後、実際に収集されたイベントの確実/探索の分類結果を見ながら、
判定ロジックを調整する。

**レビューした運用結果**: 2026-08-08〜13の実データ101件（エヴァンゲリオン/バイオハザード/
ゼルダの伝説/ジブリの4キーワード）を`matched_via`ごとにグルーピングして目視レビューした。

**発見した誤分類パターン**:
1. **略称・表記ゆれの不整合**: 「エヴァ」は正しくdirect扱いされる一方、
   「スタジオジブリ」（＝「ジブリ」の正式名称）が誤ってexpanded扱いされていた
   （「スタジオジブリ企画制作『白隠さんの禅』京都展」）。同じ「表記ゆれ」のはずなのに
   キーワードによって扱いが割れていた
2. **関連語検索経由でもタイトルに作品名がある場合の誤判定**: 「綾波レイ」で検索して
   見つけた「ヱヴァンゲリヲン新劇場版 綾波レイ 1/7 完成品フィギュア」や、
   「貞本義行」で検索して見つけた「貞本義行画集 EVANGELION【限定版】」が、
   タイトルに作品名が明記されているにもかかわらずexpanded扱いされていた。
   旧プロンプトの文言自体は「ページの内容が登録キーワードそのものについてならdirect」
   だったが、Geminiが実際には「どの検索クエリ経由で見つかったか」で判定してしまう
   傾向が見られた

**対応**: `buildExtractionPrompt`のmatched_via/matched_term判定セクションを、
「どの検索で見つかったかではなく、イベント自体の内容が登録キーワードを直接指しているか」
という基準を明示し、上記2パターンを具体例として追加する形で書き直した。

**Acceptance criteria:**
- [x] 数日分の運用結果をレビューし、誤分類（探索なのに確実、逆も）の傾向を洗い出している
- [x] 判定基準を `web/lib/gemini-prompt.ts`（`buildExtractionPrompt`）に反映し、
      改善が確認できる（ユニットテストで新基準の文言を保証）

**Verification:**
- [x] ユニットテスト1件追加（`gemini-prompt.test.ts`）、全体で179件パス。`npm run build`成功
- [ ] チューニング後の数日間で、誤分類の体感が減っている（定性確認）。今後の自動収集結果を
      継続的に見て判断する（即日には確認できない性質の検証のため保留）
...
### Task 19: Web画面のデザイン改善 — **完了（2026-08-10）**
...
**実装メモ**: `/settings` はTask 11未着手のため対象外（着手時に本デザインを踏襲する）。
配色ロジック（キーワード別カテゴリ色の決定的割当 `lib/colors.ts`、日付の緊急度判定
`lib/urgency.ts`）は純粋関数としてユニットテストを追加した上で実装。ダークモードは
トグルUIを設けず、`prefers-color-scheme`によるTailwind標準の自動切り替えとした
（意図ドキュメント上マルチユーザー対応がスコープ外であり、個人利用のOS設定に
委ねるだけで十分なため）。

**Acceptance criteria:**
- [x] Tailwind CSS が導入され、全画面に適用されている
- [x] モバイル幅（375px）で崩れずに閲覧できる
- [x] イベント0件・キーワード0件時に、適切な空状態メッセージが表示される
- [x] ダークモードで表示が破綻しない

**Verification:**
- [x] ユニットテスト13件追加（`colors.test.ts`4件、`urgency.test.ts`9件）、全体で104件パス
- [x] ローカルdevサーバー + ブラウザで実データ（30件）を用い、モバイル幅（375px）・
      デスクトップ幅、ライト/ダーク両モードで `/`・`/events`・`/events/[id]`・`/keywords`
      を目視確認。ナビゲーションのラベル折り返しを1件修正済み
- [x] `npm run build` 成功
