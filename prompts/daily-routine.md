...
**ローカル環境で `claude -p` により実行する指示**（[scripts/run-daily-routine.sh](../scripts/run-daily-routine.sh)
経由、毎日07:00 JSTにlaunchdから起動）。

> 2026-08-08: 当初はAnthropicのクラウドエージェントルーチン（RemoteTrigger）で実行する
> 想定だったが、実行環境のegressポリシーによりVercelアプリへの通信がブロックされ、
> 動作しないことが判明した（詳細: [secrets-handling.md](../docs/research/secrets-handling.md)）。
> そのためローカル実行に切り替えている。

## 環境変数（実行前に設定されている前提）

このプロンプトを実行するシェルには、以下の環境変数が事前に設定されている
（[scripts/run-daily-routine.sh](../scripts/run-daily-routine.sh)が`web/.env.local`から読み込む）。
Bashツールで直接参照すること（例: `curl -H "Authorization: Bearer $INTERNAL_INGEST_TOKEN" ...`）。
値をこのファイルや会話に書き出さないこと。

- `$APP_URL` — VercelにデプロイしたWebアプリのURL
- `$INTERNAL_INGEST_TOKEN` — `/api/ingest`・`/api/search/youtube` 認証用の内部トークン
- `$DRY_RUN` — `true` または `false`
