...
## matched_via / matched_term の判定（重要）

判定基準は**どの検索で見つかったかではなく、そのイベント自体の内容が
登録キーワード「${keyword}」を直接指しているかどうか**です。

以下のいずれかに該当すれば matched_via: "direct"、matched_term: "${keyword}" にしてください:
- イベントのタイトル・本文に「${keyword}」自体が明記されている
- 「${keyword}」の略称・表記ゆれ・正式名称違いが使われている
  （例:「エヴァンゲリオン」に対する「エヴァ」「EVANGELION」「ヱヴァンゲリヲン」、
  「ジブリ」に対する「スタジオジブリ」。これらは別物ではなく同一キーワードとして扱う）
- 声優名・キャラクター名・スタジオ名などの関連語で検索して見つけたページでも、
  その商品・イベント自体のタイトルに「${keyword}」（または上記の略称・表記ゆれ）が
  明記されていれば matched_via は "direct" にする
  （例:「ヱヴァンゲリヲン新劇場版 綾波レイ フィギュア」は「綾波レイ」で検索して
  見つけたとしても、タイトルに作品名が入っているので direct）

matched_via: "expanded" にするのは、イベント自体のタイトル・本文のどこにも
「${keyword}」（略称・表記ゆれ含む）が現れず、声優・キャラクター名・コラボ相手・
地域プロジェクト名等の関連語だけでつながっている場合のみです。matched_term には
実際の関連語を入れてください
（例:「渚カヲル写真集」はタイトルに「エヴァンゲリオン」が無く「渚カヲル」のみなので expanded）
