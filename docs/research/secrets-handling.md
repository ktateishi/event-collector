...
### 抜け漏れ（再現率）改善（2026-08-09）

URL精度は改善したが、ユーザーから「Googleで直接『バイオハザード イベント』と検索すれば
一番上に出てくるイベント（THE WORLD OF BIOHAZARD 30周年展）が拾えていない」との
指摘があった。原因は、検索クエリの決定を完全にモデルの自由判断に任せていたため、
最も基本的で当たり前のクエリすら実行されないことがあったこと。

**対処**: 検索を2種類に分離した。

1. **定番クエリ検索**（`buildCoreSearchPrompt`）: 「キーワード + イベント/展覧会/コラボ/
   グッズ/チケット」という基本パターンを**コード側で明示的に列挙**し、モデルには
   「これらを全部実行しろ」と指示するだけにする。モデルの判断に依存しないため、
   最も基本的な検索を取りこぼさない
2. **拡張検索**（`buildExpansionSearchPrompt`）: 従来通り、声優・スタジオ・コラボ相手等の
   関連語探しはモデルの知識・判断に任せる（これはコード側で列挙できない）

両方の検索結果を統合・重複除去し、URL上限を6件→12件に引き上げた
（クエリの種類が増えた分、取りこぼしを減らすため）。

**検証結果**: 「バイオハザード」で再テストしたところ、「THE WORLD OF BIOHAZARD 30周年展」
（PR TIMES・ぴあ・ファミ通・Capcom公式サイト・eeo media等、複数ソース）が
正しく収集されるようになった。収集件数も従来の14件→39件に増加。
