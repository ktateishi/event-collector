...
**この方式自体は2026-08-08中にさらに不採用となった**（下記「再度の方針転換」参照）。
...
`web/lib/gemini-prompt.ts`（プロンプト生成・レスポンス解析）と `web/lib/gemini.ts`
（GCP認証・Vertex AI呼び出し）を実装し、`/api/cron/collect`（Vercel Cron想定エンドポイント）
経由で実際のGCPプロジェクトに対して動作確認した。

**判明した制約:**
- このGCPプロジェクトのVertex AIでは、事前調査で想定していた **Gemini 3系モデルが
  未提供**（`gemini-3-flash`, `gemini-3.6-flash`等はすべて404）。代わりに
  `gemini-2.5-flash` を使用している
- `gemini-2.5-flash` では **Search Tool（Google検索グラウンディング）と構造化出力
  （`responseSchema`/`responseMimeType: "application/json"`）を同一リクエストで
  併用できない**（400エラー "controlled generation is not supported with Search tool"）。
  これは事前調査で「Gemini 3以降は両立可能」と分かっていた制約が、Gemini 3が
  未提供のこのプロジェクトではそのまま該当したことを意味する
- 対応: 構造化出力の強制指定をやめ、プロンプト内の指示文（「JSON形式で出力して」）のみに
  頼り、レスポンステキストを緩い解析（コードフェンス除去・寛容なJSONパース）で
  `CandidateEvent[]` に変換する方式にした。実際に「呪術廻戦」「ワンピース」で
  テストし、各7〜8件の妥当な候補が収集され、Supabaseへの書き込みも成功した

**今後の改善余地**: Gemini 3系がこのプロジェクトで利用可能になった場合、
構造化出力の併用を再検討する価値がある（パース失敗のリスクをさらに下げられる）。
また、単発API呼び出しには「開催が終了間近・経過済みのイベントを除外する」判断が
ローカルAIエージェント版ほど徹底されない傾向が見られた（Task 16のノイズ判定基準
調整で対応する）。

### 精度改善（2026-08-09）: URL幻覚・リンク切れ問題への対処

実運用テストで、モデルが**文章として書いたURL**の多くがリンク切れ・誤ページに
なっていることが判明した（ユーザー報告）。実データで原因を特定したところ:

- `groundingChunks[].web.uri` は `vertexaisearch.cloud.google.com/grounding-api-redirect/...`
  という中継URLで、302の`Location`ヘッダに実際のURLが入っている（実在することを確認済み）
- しかし旧実装は検索結果の**テキスト出力**（モデルが要約時に書いたURL文字列）を
  そのまま使っており、これがモデルの記憶による幻覚・複数ページの取り違えの原因だった

**対処: 3段階パイプラインに再設計**（1回のAPI呼び出しから3段階へ）

1. `gemini-search.ts`: Google検索グラウンディングで`groundingChunks`から中継URLを回収し、
   302の`Location`ヘッダを読んで実URLに解決（最大6件/キーワード）
2. `fetch-page.ts`: 実URLをブラウザ相当のUser-Agentで実際にfetchし、HTML本文をテキスト化
   （取得失敗・404のページはこの時点で除外される）
3. `gemini-extract.ts`: 実際に取得できたページ本文だけを渡し、構造化抽出
   （Search Toolを使わないためresponseSchemaを併用できる）

さらに、抽出結果のURLも**モデルに文字列として書かせず**、渡したページの
「ページ番号(page_id)」を返させてコード側で実URLに変換する方式にした
（`parseGeminiCandidates`）。範囲外のpage_idが返った場合はそのイベントごと破棄する。
これにより、複数ページを同時に渡した際の取り違えによる誤リンクも防いでいる。

**Vertex AI responseSchemaの追加の制約（判明した実装上の注意点）**:
標準のJSON Schemaで書ける `type: ["string", "null"]`（union型）はVertex AIの
schemaパーサーでは400エラーになる。代わりに `{ type: "string", nullable: true }`
という独自拡張構文を使う必要がある。

**検証結果**: 修正前は収集した17件中5件（約29%）がリンク切れ・誤ページだったのに対し、
修正後は同様のテストで収集した全件（4件）が実在するページ（HTTP 200）だった。
